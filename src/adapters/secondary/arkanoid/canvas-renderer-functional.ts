...
export function createCanvasRenderer(
  ...
  return {
    ...
    drawPaddle: (paddle: Paddle) => {
      ...
      context.quadraticCurveTo(paddle.x + paddle.width, paddle.y, paddle.x + paddle.width, paddle.y + radius);
      context.lineTo(paddle.x + paddle.width, paddle.y + paddle.height - radius);
      context.quadraticCurveTo(paddle.x + paddle.width, paddle.y + paddle.height, paddle.x + paddle.width - radius, paddle.y + paddle.height);
      context.lineTo(paddle.x + radius, paddle.y + paddle.height);
      context.quadraticCurveTo(paddle.x, paddle.y + paddle.height, paddle.x, paddle.y + paddle.height - radius);
      ...
      for (let c = 0; c < columnCount; c++) {
        for (let r = 0; r < rowCount; r++) {
          ...
          if (brick.status === 1) {
            ...
            const brickWidth = c > 0 ? 
              bricks[c][r].x - bricks[c-1][r].x : 
              (c+1 < columnCount ? bricks[c+1][r].x - bricks[c][r].x : 50);
            
            const brickHeight = r > 0 ? 
              bricks[c][r].y - bricks[c][r-1].y - 10 : 
              (r+1 < rowCount ? bricks[c][r+1].y - bricks[c][r].y - 10 : 20);
